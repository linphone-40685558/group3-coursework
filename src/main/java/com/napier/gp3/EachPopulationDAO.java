...
public class EachPopulationDAO {
    ...
    public List<Population> getPopulationByEachContinent() {
        ...
        try {
            ...
            System.out.println("Failed to get population by each continent: " + e.getMessage());
        }
        return populations;
    }
    ...
    public List<Population> getPopulationByEachRegion() {
        ...
        try {
            ...
            System.out.println("Failed to get population by each region: " + e.getMessage());
        }
        return populations;
    }
    ...
    /**
     * get population for every country
     *
     * @return
     */

    public List<Population> getPopulationByEachCountry() {
        List<Population> populations = new ArrayList<>();

        try {
            // first query for total population
            String strSelect1 = "SELECT country.Name AS Country, country.Population AS TotalPopulation " +
                    "FROM country";
            ...
            while (rset1.next()) {
                ...
                // second query for urban population
                String strSelect2 = "SELECT SUM(city.Population) AS UrbanPopulation FROM city JOIN country ON city.CountryCode = country.Code WHERE country.Name = ? ";
                ...
                if (ruralPopulation < 0) {
                    ruralPopulation = 0;
                }
                // Percentage calculation for urban population and rural population
                if (totalPopulation > 0 && ruralPopulation > 0) {
                    urbanPercentage = ((float) urbanPopulation / totalPopulation) * 100;
                    ruralPercentage = ((float) ruralPopulation / totalPopulation) * 100;
                }
                populations.add(new Population(country, totalPopulation, urbanPopulation, ruralPopulation, urbanPercentage, ruralPercentage));
            }
        } catch (SQLException e) {
            System.out.println("Failed to get population by each country: " + e.getMessage());
        }
        return populations;
    }
}
